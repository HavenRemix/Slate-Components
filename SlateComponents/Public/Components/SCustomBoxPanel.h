 ...
 * 
 */
class SCustomBoxPanel : public SPanel
{
public:
...
protected:
	 ...
	 */
	SCustomBoxPanel(EOrientation InOrientation);
...
/** A Horizontal Box Panel. See SCustomBoxPanel for more info. */
class SCustomHorizontalBox : public SCustomBoxPanel
{
public:
	class FSlot : public SCustomBoxPanel::FSlot
	{
	public:
		FSlot()
			: SCustomBoxPanel::FSlot()
		{
		}
		...
		{
			SCustomBoxPanel::FSlot::operator[](InWidget);
			return *this;
		}
	...
	SLATE_BEGIN_ARGS(SCustomHorizontalBox)
	{
		_Visibility = EVisibility::SelfHitTestInvisible;
	}

	SLATE_SUPPORTS_SLOT(SCustomHorizontalBox::FSlot)

		SLATE_END_ARGS()

		FSlot& AddSlot()
	{
		SCustomHorizontalBox::FSlot& NewSlot = *new SCustomHorizontalBox::FSlot();
	...
	FORCENOINLINE SCustomHorizontalBox()
		: SCustomBoxPanel(Orient_Horizontal)
	{
		SetCanTick(false);
...
/** A Vertical Box Panel. See SCustomBoxPanel for more info. */
class SCustomVerticalBox : public SCustomBoxPanel
{
public:
	class FSlot : public SCustomBoxPanel::FSlot
	{
	...
	SLATE_BEGIN_ARGS(SCustomVerticalBox)
	{
		_Visibility = EVisibility::SelfHitTestInvisible;
	}

	SLATE_SUPPORTS_SLOT(SCustomVerticalBox::FSlot)

		SLATE_END_ARGS()

		FSlot& AddSlot()
	{
		SCustomVerticalBox::FSlot& NewSlot = *new SCustomVerticalBox::FSlot();
		this->Children.Add(&NewSlot);
	...
	FORCENOINLINE SCustomVerticalBox()
		: SCustomBoxPanel(Orient_Vertical)
	{
		SetCanTick(false);
		bCanSupportFocus = false;
	}
...
class FCustomDragAndDropVerticalBoxOp : public FDragDropOperation
{
public:
	DRAG_DROP_OPERATOR_TYPE(FCustomDragAndDropVerticalBoxOp, FDragDropOperation)

		/** Data this item represent */
		int32 SlotIndexBeingDragged;
	SCustomVerticalBox::FSlot* SlotBeingDragged;

	virtual ~FCustomDragAndDropVerticalBoxOp()
	{}
};

/** A Vertical Box Panel. See SCustomBoxPanel for more info. */
class SCustomDragAndDropVerticalBox : public SCustomVerticalBox
{
public:
	/**
	...
	*/
	...
	DECLARE_DELEGATE_RetVal_FourParams(FReply, FOnDragAndDropVerticalBoxDragDetected, const FGeometry&, const FPointerEvent&, int32, SCustomVerticalBox::FSlot*)
	...
	DECLARE_DELEGATE_RetVal_ThreeParams(TOptional<EItemDropZone>, FOnCanAcceptDrop, const FDragDropEvent&, SCustomDragAndDropVerticalBox::EItemDropZone, SCustomVerticalBox::FSlot*);
	/** Delegate signature for handling the drop of FDragDropEvent onto target of type ItemType */
	DECLARE_DELEGATE_RetVal_FourParams(FReply, FOnAcceptDrop, const FDragDropEvent&, SCustomDragAndDropVerticalBox::EItemDropZone, int32, SCustomVerticalBox::FSlot*);

	SLATE_BEGIN_ARGS(SCustomDragAndDropVerticalBox)
	{}
	...
	*/
	...
	SCustomDragAndDropVerticalBox& SetDropIndicator_Above(const FSlateBrush& InValue) { DropIndicator_Above = InValue; return *this; }
	SCustomDragAndDropVerticalBox& SetDropIndicator_Below(const FSlateBrush& InValue) { DropIndicator_Below = InValue; return *this; }
...
private:
	/** @see SCustomDragAndDropVerticalBox's OnCanAcceptDrop event */
	FOnCanAcceptDrop OnCanAcceptDrop;

	/** @see SCustomDragAndDropVerticalBox's OnAcceptDrop event */
