 ...
 */
void SCustomHorizontalBox::Construct(const SCustomHorizontalBox::FArguments& InArgs)
{
 ...
 * @param	InArgs	The declaration data for this widget
 */
void SCustomVerticalBox::Construct(const SCustomVerticalBox::FArguments& InArgs)
{
	const int32 NumSlots = InArgs.Slots.Num();
	for (int32 SlotIndex = 0; SlotIndex < NumSlots; ++SlotIndex)
	{
...
static void ArrangeChildrenAlong(EFlowDirection InLayoutFlow, const TPanelChildren<SCustomBoxPanel::FSlot>& Children, const FGeometry& AllottedGeometry, FArrangedChildren& ArrangedChildren)
{
	...
	if (Children.Num() > 0)
	{
		...
		{
			const SCustomBoxPanel::FSlot& CurChild = Children[ChildIndex];
		...
		for (TPanelChildrenConstIterator<SCustomBoxPanel::FSlot> It(Children, Orientation, InLayoutFlow); It; ++It)
		{
			const SCustomBoxPanel::FSlot& CurChild = *It;
 ...
 */
void SCustomBoxPanel::OnArrangeChildren(const FGeometry& AllottedGeometry, FArrangedChildren& ArrangedChildren) const
{
 ...
 */
template<EOrientation Orientation>
static FVector2D ComputeDesiredSizeForBox(const TPanelChildren<SCustomBoxPanel::FSlot>& Children)
{
 ...
 */
FVector2D SCustomBoxPanel::ComputeDesiredSize(float) const
{
...
FChildren* SCustomBoxPanel::GetChildren()
{
	return &Children;
}

int32 SCustomBoxPanel::RemoveSlot(const TSharedRef<SWidget>& SlotWidget)
{
...
void SCustomBoxPanel::ClearChildren()
{
	Children.Empty();
 ...
 */
SCustomBoxPanel::SCustomBoxPanel(EOrientation InOrientation)
	: Children(this)
...
void SCustomDragAndDropVerticalBox::Construct(const FArguments& InArgs)
{
	SCustomVerticalBox::Construct(SCustomVerticalBox::FArguments());
...
FReply SCustomDragAndDropVerticalBox::OnMouseButtonDown(const FGeometry& MyGeometry, const FPointerEvent& MouseEvent)
{
...
FReply SCustomDragAndDropVerticalBox::OnDragDetected(const FGeometry& MyGeometry, const FPointerEvent& MouseEvent)
{
	...
	{
		SCustomVerticalBox::FSlot* Slot = &(*PanelChildren)[NodeUnderPositionIndex];
...
void SCustomDragAndDropVerticalBox::OnDragEnter(FGeometry const& MyGeometry, FDragDropEvent const& DragDropEvent)
{
...
void SCustomDragAndDropVerticalBox::OnDragLeave(FDragDropEvent const& DragDropEvent)
{
...
SCustomDragAndDropVerticalBox::EItemDropZone SCustomDragAndDropVerticalBox::ZoneFromPointerPosition(FVector2D LocalPointerPos, const FGeometry& CurrentGeometry, const FGeometry& StartGeometry) const
{
...
FReply SCustomDragAndDropVerticalBox::OnDragOver(const FGeometry& MyGeometry, const FDragDropEvent& DragDropEvent)
{
	if (OnCanAcceptDrop.IsBound())
	{
		...
		TSharedPtr<FCustomDragAndDropVerticalBoxOp> DragOp = DragDropEvent.GetOperationAs<FCustomDragAndDropVerticalBoxOp>();
		...
		if (DragOp.IsValid())
		{
			...
			if (ArrangedChildren.IsValidIndex(DragOverSlotIndex))
			{
				...
				{
					SCustomVerticalBox::FSlot* Slot = &(*PanelChildren)[DragOverSlotIndex];
...
FReply SCustomDragAndDropVerticalBox::OnDrop(const FGeometry& MyGeometry, const FDragDropEvent& DragDropEvent)
{
	...
	if (OnAcceptDrop.IsBound())
	{
		...
		if (DragDropEvent.GetOperationAs<FCustomDragAndDropVerticalBoxOp>().IsValid())
		{
...
int32 SCustomDragAndDropVerticalBox::OnPaint(const FPaintArgs& Args, const FGeometry& AllottedGeometry, const FSlateRect& MyCullingRect, FSlateWindowElementList& OutDrawElements, int32 LayerId, const FWidgetStyle& InWidgetStyle, bool bParentEnabled) const
{
	FArrangedChildren ArrangedChildren(EVisibility::Visible);
